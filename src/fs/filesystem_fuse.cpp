...
static auto GetAttr(const char* path, fuse_wrapper::stat* stat, fuse_file_info*) -> int {
    ...
    const auto res = GetFs()->getAttributes(path, std::addressof(entry));
    fuse_wrapper::FillStat(*stat, entry.createTime);
    if (entry.type == Type::Directory) {
        stat->st_mode |= S_IFDIR;
    } else {
        stat->st_mode |= S_IFREG;
...
static auto StatFs(const char* path, fuse_wrapper::statvfs* statfs) -> int {
    ...
    std::memset(statfs, 0, sizeof(*statfs));
    statfs->f_flag |= R_OK;
    return SUCCESS;
}
...
static auto ReadDir(const char* path, void* buf, fuse_fill_dir_t filler, fuse_wrapper::off_t /* offset */, fuse_file_info* info, fuse_readdir_flags /* flags */) -> int {
    ...
    std::size_t count;
    if (const auto res = dir->getCount(std::addressof(count)); NX_FAILED(res)) {
        return res;
    }

    auto entries = std::vector<DirectoryEntry>(count);
    std::size_t read;
    if (const auto res = dir->read(std::addressof(read), entries.data(), entries.size(), 0); NX_FAILED(res)) {
        return res;
    }
